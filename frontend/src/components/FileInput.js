...
function FileInput() {
	...
	async function handleSubmit(event) {
		...
		setLoading(true);
		axios
			.post(url, formData, configHeaders)
			.then((response) => {
				...
				dispatch(closeModal());
				toast.success(
					"File uploaded successfully, Now you can start chatting with Document"
				);
			})
			.catch((error) => {
			...
			<div className="flex justify-center items-center mb-[3rem] mt-[3rem]">
				<div className="max-w-screen-xl w-full px-4">
					<div className="max-w-md mx-auto bg-white p-6 rounded-md shadow-md">
						<div
							className={`max-w-md mx-auto p-2 bg-white shadow-md rounded-md border-dashed border-2 border-[#5E5ADB] mt-5 ${
								!loading ? "relative cursor-pointer" : ""
							} h-40 z-10`}
							onDrop={handleDrop}
						...
						>
							...
							>
								<span className="text-sm">
									Click or drag & drop to attach PDF files
								</span>
								<input
									type="file"
									id="fileInput"
